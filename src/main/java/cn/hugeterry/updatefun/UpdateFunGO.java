...
import cn.hugeterry.updatefun.module.UpdateListener;
...
public class UpdateFunGO {
    ...
    public static UpdateFunGO init(Context context, String updateUrl,UpdateListener listener) {
    ...
    private UpdateFunGO(Context context) {
        ...
        if (DownloadKey.TOShowDownloadView != 2) {
            thread_update = new Thread(new HandleUpdateResult(context,mUpdateUrl,mUpdateListener));
            thread_update.start();
        }
    }
